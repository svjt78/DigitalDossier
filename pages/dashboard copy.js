...
export async function getServerSideProps() {
  try {
    ...
    const mapUrls = (item) => ({
      ...item,
      coverUrl: item.coverKey ? buildUrl(imgPrefix, item.coverKey) : null,
      pdfUrl: item.pdfKey ? buildUrl(pdfPrefix, item.pdfKey) : null,
    });
  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto p-4 sm:p-8">
        ...
        {manageModalOpen && <ManageSubscriptionsModal onClose={() => setManageModalOpen(false)} />}
      </div>
    </>
  );
}
