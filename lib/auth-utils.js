...
export async function getUserFromToken(req) {
  ...
  try {
    ...
    if (!user && (payload.email || payload.user_email)) {
      ...
      // If found by email but missing credential_user_id, update it
      if (user && !user.credential_user_id) {
        console.log('🔍 Updating user with credential_user_id');
        await prisma.user.update({
          where: { id: user.id },
          data: { credential_user_id: credentialUserId }
        });
        user.credential_user_id = credentialUserId;
      }
    }
    ...
    console.log('🔍 Found local user:', { 
      ...
      credentialId: user.credential_user_id 
    });
    ...
    return {
      ...
      email: user.email,
      username: user.name,
      credentialUserId: user.credential_user_id
    };
  } catch (error) {
