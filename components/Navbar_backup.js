...
export default function Navbar() {
  ...
  // if redirected here with ?redirect=subscribe and already logged in, auto-open modal
  useEffect(() => {
  ...
  const handleSubscribeClick = () => {
    if (!isAuthenticated) {
      // not logged in → send to login page, preserving the intent to subscribe
      router.push('/login?redirect=subscribe');
    } else {
      // already logged in → open subscription form
      setShowSubscribe(true);
    }
  };
